...
function AutoReply(type, msg) {
	...
	$('#msg_client').val('');
	if (type == 'offline') {
		reply("Pardon, il n'y a personne en ligne maintenant.");
		reply(
			"Vous pouvez lassier votre message <a style='text-decoration: underline; color: #ff9900' href='contact.php'>EN CLIQUANT ICI</a>"
		);
		reply("Nous vont vous repondre des que possible.");
	} else if (type == 'disconnect') {
		reply("Pardon, l'internet est bloqué.");
		reply(
			"Vous pouvez réessayer plus tard ou lassier votre message <a style='text-decoration: underline; color: #ff9900' href='contact.php'>EN CLIQUANT ICI</a>"
		);
	}
...
$(window).keydown(function(evt) {
	if (evt.keyCode == 13) {
		if (online == '1') {
			...
			AutoReply('offline', ""); //if no one is online, lead client to leave a msg
		} else if (online == '-1') {
			AutoReply('disconnect', "");
		}
	}
});
